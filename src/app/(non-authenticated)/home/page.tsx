...
import SpotifySongSuggestion from '@/designSystem/ui/SpotifySongSuggestion'

const { Title } = Typography

const HomePage = () => {
  const [selectedMood, setSelectedMood] = useState<'Peaceful' | 'Motivated' | 'Happy'>('Peaceful')

  return (
    <PageLayout layout="narrow">
      <div className="text-center mb-8">
        <Title level={1}>Song Suggestions Based on Your Mood</Title>
        <div className="flex justify-center space-x-4">
          <Button
            type={selectedMood === 'Peaceful' ? 'primary' : 'default'}
            onClick={() => setSelectedMood('Peaceful')}
          >
            Peaceful 🍃
          </Button>
          <Button
            type={selectedMood === 'Motivated' ? 'primary' : 'default'}
            onClick={() => setSelectedMood('Motivated')}
          >
            Motivated 🚀
          </Button>
          <Button
            type={selectedMood === 'Happy' ? 'primary' : 'default'}
      ...
      <Row gutter={[16, 16]}>
        <Col span={24}>
          <SpotifySongSuggestion mood={selectedMood} />
        </Col>
      </Row>
    </PageLayout>
  )
...
const SpotifySongSuggestion: React.FC<SpotifySongSuggestionProps> = ({ mood }) => {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
      {tracks?.map((track) => (
        <Card key={track.id} className="relative mb-4" cover={<img alt={track.name} src={track.album.images[0].url} />}>
